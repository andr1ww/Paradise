...
namespace Memcury
{
	...
	public:
		...
		static auto FindPtrRef(void *pointer) -> Scanner
		{
			PE::Address addr{nullptr};

			auto rdata = PE::Section::GetSection(".rdata");
			const auto bytes = reinterpret_cast<std::uint8_t *>(rdata.GetSectionStart().Get());

			for (DWORD i = 0; i < rdata.GetSectionSize(); i++)
			{
				auto ptr = *reinterpret_cast<void **>(bytes + i);

				if (ptr == pointer)
				{
					addr = PE::Address(&bytes[i]);
					break;
				}
			}
	...
	namespace VEHHook
	{
		struct HOOK_INFO
		{
			...
			HOOK_INFO(void *Original, void *Detour)
				: Original(Original), Detour(Detour)
			{
			}
		};
